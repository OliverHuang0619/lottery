...
export default function Workspace() {
  ...
  async function login() {
    try {
      ...
      if (!response.ok) throw new Error(loginResult.error || `HTTP ${response.status}`)
      sessionStorage.setItem('lottery-session', loginResult.sessionToken)
      const rows = await api<Conversation[]>('/api/conversations')
      setConversations(rows); setAuthenticated(true); setView('dashboard'); setPassword(''); setError('')
      if (rows[0]) setSelected(rows[0].id)
    } catch (e) { setError(String(e)) }
  }
  useEffect(() => {
    if (!sessionStorage.getItem('lottery-session')) return
  ...
  async function logout() {
    try { await api('/api/logout', { method: 'POST' }) } catch { /* discard an expired server session */ }
    sessionStorage.removeItem('lottery-session')
    setAuthenticated(false); setView('dashboard'); setTasks([]); setPassword('')
  }
