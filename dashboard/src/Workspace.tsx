...
import { ArrowUp, BarChart3, CirclePlus, LogOut, MessageSquare, Radio, Sparkles } from 'lucide-react'
import App from './App'
...
type AppConfig = { defaultSource: string; defaultModel: string; defaultEffort: string; models: Option[]; efforts: Option[] }
...
export default function Workspace() {
  ...
  async function login() {
    ...
    try {
      ...
      setConversations(rows); setAuthenticated(true); setError('')
  ...
  useEffect(() => {
    ...
    api<Conversation[]>('/api/conversations').then(rows => {
      setConversations(rows); setAuthenticated(true)
      if (rows[0]) setSelected(rows[0].id)
    }).catch(e => setError(String(e)))
  }, [])
  useEffect(() => {
  ...
  async function send(quick = false) {
    ...
    try {
      ...
      const task = await api<Task>(`/api/conversations/${id}/messages`, { method: 'POST', body: JSON.stringify({ content, skill: 'analyze-lottery-history', model, reasoningEffort, ...(quick ? { sourceUrl: source } : {}) }) })
