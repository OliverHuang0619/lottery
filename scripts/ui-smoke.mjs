...
const token = (await readFile(path.join(root, '.env'), 'utf8')).match(/^APP_TOKEN=(.+)$/m)?.[1].trim()
...
try {
  ...
  await page.getByRole('heading', { name: '分析对话', exact: true }).waitFor()
  await page.screenshot({ path: path.join(output, 'desktop.png'), fullPage: true })
  await page.getByRole('button', { name: '数据仪表板', exact: true }).click()
